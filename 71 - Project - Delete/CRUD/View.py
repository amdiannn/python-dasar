...
def read_console():
    ...
    # Header
    print("\n"+"="*100)
    print(f"{index:4} | {judul:45} | {penulis:35} | {tahun:5}")
    print("-"*100)
    ...
    for index,data in enumerate(data_file):
        ...
        judul = data_break[3]
        tahun = data_break[4]
        print(f"{index+1:<4} | {judul:.45} | {penulis:.35} | {tahun:4}", end="")
...
def update_console():
    ...
    while(True):
        ...
        print(f"1. Judul\t: {judul:.45}")
        print(f"2. Penulis\t: {penulis:.35}")
...
def delete_console():
    ...
    while(True):
        print("\n"+"="*100)
        print("Silakan Pilih Nomor Buku Untuk Didelete\n")
        no_buku = int(input("Nomor Buku: "))
        data_buku = Operasi.read(index=no_buku)
        ...
        if data_buku:
            ...
            print("\n"+"="*100)
            print("Cek Database Yang Akan Didelete:\n")
            print(f"1. Judul\t: {judul:.45}")
            print(f"2. Penulis\t: {penulis:.35}")
            print(f"3. Tahun\t: {tahun:4}")

            isDone = input("\nYakin Delete Database? (y/n) ")
            if isDone == "y" or isDone == "Y":
